package tex_visual_v2 {
    ...
    public class TeX_Renderer {
        ...
        public function TeX_Renderer(fnt:TeX_TiledFont) {
            font = fnt
        ...
        private function mergeInLine(vec:Vector.<TeX_SpriteContainer>):TeX_SpriteContainer {
            if (vec.length == 0) {
                return formChar(font.retrieveDataForChar("err"))
            }
            if (vec.length == 1) {
                return vec[0]
            }
         ...
         * @return
         */
        private function mergeVertically(vec:Vector.<TeX_SpriteContainer>, baselineOriginIndex:uint, align:String = TextFormatAlign.LEFT, snap_grid_width:uint = 1):TeX_SpriteContainer {
        ...
        private function formFrac(token:TeX_Token):TeX_SpriteContainer {
            var down:TeX_SpriteContainer = mergeInLine(batchForm(token.args[1]))
            var up:TeX_SpriteContainer = mergeInLine(batchForm(token.args[0]))
            var fracLinePiece:TeX_Sprite = formChar(font.retrieveDataForChar("fraction_bar")).pieces[0]
            var wt:uint = Math.max(down.width, up.width) / fracLinePiece.width
            var pieces:Vector.<TeX_Sprite> = new Vector.<TeX_Sprite>
            for (var i:int = 0; i < wt; i++) {
                var pc:TeX_Sprite = fracLinePiece.copy()
                pc.position.x = fracLinePiece.width * pieces.length
                pieces.push(pc)
            }
            var fracline:TeX_SpriteContainer = new TeX_SpriteContainer(pieces, 0)
            
            return mergeVertically(new <TeX_SpriteContainer>[up, fracline, down], 1, TextFormatAlign.CENTER, font.tileWidth)
        }
